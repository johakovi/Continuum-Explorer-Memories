...
fun openFile(context: Context, file: UniversalFile, originalFile: UniversalFile? = null) {
    ...
    // Use internal PDF Viewer (requires API 28+)
    if ((extension == "pdf" || mimeType == "application/pdf") && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        val intent = Intent(context, com.troikoss.continuum_explorer.ui.activities.PdfViewerActivity::class.java).apply {
            setData(uri)
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
